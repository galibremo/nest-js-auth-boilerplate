  ...
  if (opts?.min)
    schema = schema.min(opts.min, {
      error: makeError(
        name,
        'limit',
        zodMessages.error.limit.stringMin(name, opts.min),
      ),
    });
  ...
  if (opts?.max)
    schema = schema.max(opts.max, {
      error: makeError(
        name,
        'limit',
        zodMessages.error.limit.stringMax(name, opts.max),
      ),
    });

  if (opts?.regex)
    schema = schema.regex(opts.regex, {
      error: opts.regexMsg || makeError(name, 'invalid'),
    });
...
const baseNumber = (
  name: string,
  opts?: { min?: number; positive?: boolean; int?: boolean },
) => {
  ...
  if (opts?.positive)
    schema = schema.positive({ error: makeError(name, 'invalid') });
...
const baseEnum = <const T extends readonly [string, ...string[]]>(
  name: string,
  values: T,
) =>
  z.enum(values, {
...
const baseDate = (name: string) =>
  z
    .union([z.string(), z.date()], {
      error: () =>
        makeError(name, 'invalid', `${name} must be a string or Date`),
    })
    .refine(
      (value) => {
...
export const validateStringOrArray = (
  name: string,
  itemOpts?: Parameters<typeof baseString>[1],
) =>
  baseUnion(name, [
...
export const validateUsernameOrEmail = baseString('Username or email', {
  min: 1,
  max: 255,
}).refine(
  (value) => {
...
const passwordSchema = (name: string) => baseString(name, { min: 8, max: 128 });
