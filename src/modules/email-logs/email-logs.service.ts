...
export class EmailLogsService {
  ...
  async createLog(params: CreateEmailLogParams): Promise<void> {
    try {
      ...
      this.logger.warn(
        `Failed to write email log: ${this.getErrorMessage(error)}`,
      );
    }
  }
  ...
  async getLogForResend(logPublicId: string): Promise<{
    ...
    return {
      ...
      toEmail: log.toEmail,
      toName: log.toName,
      emailProviderId: log.emailProviderId,
    };
  }

  async getLatestLogForProvider(
    providerId: number,
  ...
  private toResponse(log: EmailLogSchemaType): EmailLogResponse {
    return {
      id: log.publicId,
      emailProviderId: log.emailProviderId ? String(log.emailProviderId) : null,
      toEmail: log.toEmail,
      toName: log.toName,
      subject: log.subject,
      templateKey: log.templateKey,
